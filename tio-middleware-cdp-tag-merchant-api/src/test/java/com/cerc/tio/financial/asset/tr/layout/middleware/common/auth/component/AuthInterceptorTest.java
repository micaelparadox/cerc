...
class AuthInterceptorTest {
    ...
    void doFilterInternal_Should_filter_request_When_authorized() throws IOException, ServletException {
        ...
        given(request.getServletPath()).willReturn(RoleRoute.AP001.getRoute());
        var response = mock(HttpServletResponse.class);
        var filterChain = mock(FilterChain.class);
        doNothing().when(filterChain).doFilter(request, response);

        // when
        when(authService.authorizeParticipant(any())).thenReturn(Pair.of(true, "Authorized"));
        underTest.doFilterInternal(request, response, filterChain);

        // then
        verify(authService).authorizeParticipant(any());
    ...
    void doFilterInternal_Should_return_unauthorized_When_not_authorized() throws IOException, ServletException {
        ...
        // when
        when(authService.authorizeParticipant(any())).thenReturn(Pair.of(false, "Unauthorized"));
        underTest.doFilterInternal(request, response, filterChain);
    ...
    void doFilterInternal_Should_filter_When_route_isnt_productRoute() throws IOException, ServletException {
        ...
        // then
        verify(authService, never()).authorizeParticipant(any());
        verify(filterChain).doFilter(request, response);
    }
}